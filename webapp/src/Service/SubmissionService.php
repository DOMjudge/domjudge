        ...
        if ($allowedLanguages->isEmpty()) {
            ...
            if (!in_array($language, $allowedLanguages, strict: true)) {
                throw new BadRequestHttpException(
                    sprintf("Language '%s' not allowed for contest [c%d / %s].",
                        $language->getName(), $contest->getCid(), $contest->getName()));
            }
        } else {
            $allowedLanguages = $allowedLanguages->toArray();
            if (!in_array($language, $allowedLanguages, strict: true)) {
                throw new BadRequestHttpException(
                    sprintf("Language '%s' not allowed for problem [p%d / %s].",
                        $language->getName(), $problem->getProbid(), $problem->getProblem()->getName()));
            }
        }

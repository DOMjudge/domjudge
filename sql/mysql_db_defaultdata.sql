...
('timelimit_overshoot', '"1s|10%"', '0', 'string', 'Time that submissions are kept running beyond timelimt before being killed. Specify as "Xs" for X seconds, "Y%" as percentage, or a combination of both separated by one of "+|&" for the sum, maximum, or minimum of both.'),
...
('show_flags', '0', 'bool', '1', 'Show country flags on the scoreboard?'),
('show_affiliations', '1', 'bool', '1', 'Show affiliation names on the scoreboard?'),
('show_affiliation_logos', '1', 'bool', '1', 'Show affiliation logos on the scoreboard?'),
('show_pending', '1', 'bool', '1', 'Show pending submissions on the scoreboard?'),
...
('compile_penalty', '0', 'bool', '1', 'Should submissions with compiler-error incur penalty time (and show on the scoreboard)?'),
('results_prio', '{"memory-limit":99,"output-limit":99,"run-error":99,"timelimit":99,"wrong-answer":99,"no-output":99,"correct":1}', 'array_keyval', '0', 'Priorities of results for determining final result with multiple testcases. Higher priority is used first as final result. With equal priority, the first occurring result determines the final result.'),
('results_remap', '{}', 'array_keyval', '0', 'Remap testcase result, e.g. to disable a specific result type such as ''no-output''.'),
('lazy_eval_results', '0', 'bool', '0', 'Lazy evaluation of results? If enabled, stops judging as soon as a highest priority result is found, otherwise always all testcases will be judged.'),
...
('require_entry_point', '1', 'bool', '1', 'Require entry point for submissions.');

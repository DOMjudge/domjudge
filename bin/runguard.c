...
void outputtime()
{
	...
	if ( use_output ) {
		...
		if ( fclose(outputfile) ) {
			error(errno, "closing file `%s'", outputfile);
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	if ( use_user ) {
		...
		if ( ptr == NULL || runuid <= 0 )
			error(0, "illegal user specified: %d", runuid);
	}
	...
	if ( use_root ) {
		...
		if ( chdir(rootdir) )
			error(errno, "cannot chdir to `%s'", rootdir);

		/* Get absolute pathname of rootdir, by reading it. */
		if ( getcwd(cwd,MAXPATHLEN) == NULL )
			error(errno, "cannot get directory");
		if ( cwd[strlen(cwd) - 1] != '/' )
			strcat(cwd, "/");
		...
		if ( chroot(".") )
			error(errno, "cannot change root to `%s'", cwd);

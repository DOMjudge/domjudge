...
de programmeertaal, dan worden deze automatisch gedetecteerd. Ook
detecteert het de main-klasse (Java/Kotlin) of main-bestand (Python).
Voor de complete documentatie en alle opties, zie \cmd{submit --help}.
}
\item[Web-Interface]
...
\cmd{ext}. Voor Java gebruikt het de bestandsnaam als naam
van de main-klasse, en voor Kotlin geeft het de bestandsnaam
een hoofdletter en plakt er \cmd{Kt} achter.
Dit kan handmatig aangepast worden met de opties
\cmd{-p probleemnaam}, \cmd{-l taalextensie} en
\cmd{-e entry\_point}. Zie
...
Vanaf je teampagina \url{\BASEURL team} kun je op de groene \textbf{Submit}-knop

...
Command-line submission can be done by using \verb!submit.exe! from either a
command prompt or the Cygwin shell. The Cygwin shell can also be used to look
up C-function manual pages with \cmd{man <pagename>} and to compile with
\cmd{gcc} and \cmd{g++} for C and C++ respectively.
...
\verb!sampledata! containing the sample input/output data metioned in the
problem set.

...
On your desktop is a folder \cmd{NWERC Doc}, which contains the C(++) and Java
documentation.

The Cygwin shell must be used to run the command line submit client and can be
used to look up C-function manual pages with \cmd{man <pagename>}. You can also
use it to compile with \cmd{gcc} and \cmd{g++} for C and C++ respectively.
...
Store your solutions in your homedrive (H:\textbackslash). On your homedrive
you will also find a folder \verb!sampledata! containing the sample
input/output data metioned in the problem set.
...
\cmd{NWERC-documentation} on your desktop. We support Firefox as the browser.

The sample input/output data of the problem set can also be found in the folder
\verb!~/sampledata!.

Printing is done by using the \cmd{lpr} or \cmd{a2ps} commands.
...
\cmd{submit} or using the web interface.  A description of both methods
follows.
...
Do not declare any of your classes as ``\verb!public!''.
Java submissions should have exactly one class with method
``\verb!public static void main(String args[])!'',
see also the Java code example in appendix \ref{codeexamples}.

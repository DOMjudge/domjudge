...
Submitting solutions can be done
\ifcmdsubmit{
in two ways: with the command-line
program \cmd{submit} (if installed) or
...
For Java it uses the \cmd{filename} as a guess for the
main class; for Kotlin it capitalizes \cmd{filename} and appends
\cmd{Kt} to compute the guess for the main class name. For Python,
...
Click the green \textit{Submit} button at the menu bar on every page.

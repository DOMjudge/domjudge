...
It is however strongly advised that your team reads all of this manual.
There are specific details of this jury system that might become of
...
\cmd{ext}. For Java it uses the \cmd{filename} as main class;
for Kotlin it capitalizes \cmd{filename} and appends \cmd{Kt}
to compute the main class name. For Python 2 and 3, the first
filename is used as main file.

These decisions can be overruled with the options
...
must comply exactly with the specifications of the jury. See
...
The \DOMjudge jury system is fully automated.
...
\item[memory]
During execution of your program, there are \MEMLIMIT\ megabytes of
...
at its disposal. To increase stability of the jury system, \DOMjudge

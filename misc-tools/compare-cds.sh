...
CDS_VERSION="2.0.1797"
UTILS_VERSION="2.0.1797"
...
curl -k --connect-timeout 5 --max-time 10 -u admin:admin https://localhost:8443/api/contests/$DOMJUDGE_CID >/dev/null 2>&1
wait_for_quiet cds.log 10
...
curl -s -k --connect-timeout 5 --no-buffer -u admin:admin https://localhost:8443/api/contests/$DOMJUDGE_CID/event-feed > cds-events.json 2>/dev/null &
CURL_PID=$!
...
curl -s -k --connect-timeout 5 --no-buffer -u admin:admin $CCS_URL/event-feed > dj-events.json 2>/dev/null &
CURL_PID=$!
...
curl -s -k --connect-timeout 5 --no-buffer -u admin:admin https://localhost:8443/api/contests/$DOMJUDGE_CID/scoreboard > cds-scoreboard.json 2>/dev/null
echo "Get the scoreboard from domjudge"
curl -s -k --connect-timeout 5 --no-buffer -u admin:admin $CCS_URL/scoreboard > dj-scoreboard.json 2>/dev/null

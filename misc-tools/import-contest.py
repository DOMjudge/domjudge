...
def import_file(basename: str, files: List[str]):
    ...
    for file in files:
        if os.path.exists(file):
            if confirm(f"Import {basename}s from {file}?", False):
                print(f"Importing {basename}s.")
                type = file.split(".")[1]
                response = upload_file(f"/users/{basename}s", type, file)
                print(json.dumps(response, indent=4))
            else:
                print(f"Skipping {basename}s import.")
            any_matched = True
    if not any_matched:
        if len(files) == 1:
            print(f"'{files[0]}' not found, skipping {basename}s import.")
        else:
            prefix = [f"'{item}'" for item in files[:-1]]
            prefix_joined = ", ".join(prefix)
            last = f"'{files[-1]}'"
            print(
                f"Neither {prefix_joined} nor {last} found, skipping {basename}s import.")
...
import_file("group", ["groups.json", "groups.tsv"])
import_file("organization", ["organizations.json"])
import_file("team", ["teams.json", "teams2.tsv"])
import_file("account", ["accounts.json", "accounts.yaml", "accounts.tsv"])
...
if os.path.exists("contest.yaml"):
    ...
    if os.path.exists("problemset.yaml"):
        combine = True
        question = "Import contest metadata (from contest.yaml and problemset.yaml)?"
    else:
        question = "Import contest metadata (from contest.yaml)?"

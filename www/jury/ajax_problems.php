...
if (isset($_GET['fromcontest'])) {
	$problems = $DB->q("TABLE SELECT problem.probid AS id, problem.name,
			   ...
			   FROM problem
			   INNER JOIN contestproblem ON contestproblem.probid = problem.probid
			   WHERE (problem.name LIKE %c OR CONCAT('p', problem.probid) LIKE %c OR contestproblem.shortname LIKE %c)

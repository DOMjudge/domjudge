...
$newrequests    = $DB->q('SELECT c.*, cp.shortname, t.name AS toname, f.name AS fromname, co.shortname AS contestshortname
...
$oldrequests    = $DB->q('SELECT c.*, cp.shortname, t.name AS toname, f.name AS fromname, co.shortname AS contestshortname
			  FROM clarification c
			  LEFT JOIN problem p USING(probid)
			  LEFT JOIN contestproblem cp USING (probid, cid)
			  LEFT JOIN team t ON (t.teamid = c.recipient)
...
$clarifications = $DB->q('SELECT c.*, cp.shortname, t.name AS toname, f.name AS fromname, co.shortname AS contestshortname
			  FROM clarification c
			  LEFT JOIN problem p USING(probid)
			  LEFT JOIN contestproblem cp USING (probid, cid)
			  LEFT JOIN team t ON (t.teamid = c.recipient)
